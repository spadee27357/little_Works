...
nFile = Dir("\\10.91.40.40\fabsh$\cf5\製造部\組長專用\顏伯丞\圖片\SAMP Macro\" & ProductID & "\*.txt")  '設定想要處理的目錄為 C:\123, 處理的檔案副檔名為 *.txt
    ...
    Do While Len(nFile)
        If nFile <> "" Then
            File = Split(nFile, "_")
            'Macro07條件
            'If File(1) = "07" Or File(1) = "六小點.txt" Or File(1) = "版邊.txt" Or File(1) = Left(OperationID, 2) Or (CoaterID = "12" And File(1) = "R") Or (CoaterID = "13" And File(1) = "L") Then
            
            'Macro通用
            If File(1) <> "" Then 'Or File(1) = Left(OperationID, 2) Or (CoaterID = "12" And File(1) = "R") Or (CoaterID = "13" And File(1) = "L") Then
